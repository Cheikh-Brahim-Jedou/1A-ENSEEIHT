...
procedure Evaluer_Alea_TH is
   function Identite(entier : Integer ) return Integer is 
   begin
      return entier ;
   end Identite ;
   package TH_entier_entier is new TH(T_Cle => Integer ,T_Valeur => Integer , Capacite => 100 , hachage => Identite);
	...
	is
		...
		use Mon_Alea;
      Sda : T_TH;
      Tirage : Integer ; -- variable tir� aleatoirement 
      Min_Locale , Max_Locale : Integer ; -- Variable locaux permettant de determiner min et max
	 Procedure Mettre_a_jour( Cle : in Integer ; Valeur : in Integer) is

     begin
         if Cle>0 and Max_Locale < Valeur then
            Max_Locale := Valeur ;
         end if ;
         if Min_Locale > Valeur then
            Min_Locale := Valeur ;
         end if ;
     end Mettre_a_jour ;

      procedure Obtenir_Min_Max is new Pour_Chaque_TH(Mettre_a_jour) ;

   begin
      Initialiser(Sda);
      Max_Locale:=0 ;-- max_locale est initialiser a 0 car aucune fr�quence trouv� avant le debut du tirage
      Min_Locale := Taille ; -- pour mettre a jour la frequence minimale au fur et au mesure

      for i in 1..Taille loop
         Get_Random_Number(Tirage) ;
         begin
         Enregistrer(Sda , Tirage , La_Valeur(Sda , Tirage) + 1) ;
         exception
            when Cle_Absente_Exception => Enregistrer(Sda , Tirage , 1) ;-- on Enregistre la premiere occurrence
         end ;
